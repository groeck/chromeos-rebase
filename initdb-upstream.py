...
commits = subprocess.check_output(['git', 'log', '--abbrev=12', '--oneline', '--no-merges', upstream_drop])
...
commits = subprocess.check_output(['git', 'log', '--abbrev=12', '--oneline', '--no-merges', upstream_pick])
for commit in commits.splitlines():
    if commit != "":
        elem = commit.split(" ", 1)
        sha = elem[0]
